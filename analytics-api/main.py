...
class MetricsStore:
    ...
    def service_by_hour_of_day(
        ...
        """単一サービスの観測を UTC 時刻 ("00"〜"23") でグルーピングして返す。

        `service_by_status` がステータス別のレスポンス時間分布を返すのに対し、
        本メソッドは「1 日のうちどの時間帯にレコードが集まっているか」という
        周期パターンを返す。ダッシュボードで「深夜バッチ時間帯だけ degraded が
        急増する」「業務時間帯は healthy が多い」等の時刻依存の傾向を検出する
        用途を想定する。

        バケットキーは `timestamp` (Unix epoch 秒) を UTC 化した 2 桁ゼロ詰め
        時刻文字列 ("00"〜"23")。lex 昇順 = 時間順を保つため sorted で十分。
        populated-only: 観測 0 件の時刻は結果配列に含めない (`timeseries` と同じ
        スパース方針、他リポジトリの by_hour_of_day と同じ規約)。

        各バケットは以下を返す:
            hour / count / by_status (ALLOWED_STATUSES 全キーを 0 初期化)
            / avg_response_ms / min_response_ms / max_response_ms
            / p50_response_ms / p95_response_ms / p99_response_ms
            / first_seen / last_seen

        対象サービスのレコードが (since/until 範囲内に) 1 件も無い場合は
        `None` を返す（`service_by_status` と同じ規約）。
        """
        records_snapshot = self.filter(service=service, since=since, until=until)
        if not records_snapshot:
            return None

        # 時刻キー ("00"〜"23") → { times, by_status, first_seen, last_seen }
        per_hour: dict[str, dict] = {}
        for r in records_snapshot:
            # `timestamp` は Unix epoch 秒。UTC で時刻を抽出することで、
            # tz 越境時の分裂 (ローカルタイム基準の時刻ズレ) を避ける。
            dt = datetime.fromtimestamp(r.timestamp, tz=timezone.utc)
            hour_key = dt.strftime("%H")
            bucket = per_hour.get(hour_key)
            if bucket is None:
                bucket = {
                    "times": [],
...
def get_service_by_hour_of_day(
    ...
    `/metrics/services/{service_name}/by_status` がステータス別のレスポンス時間
    分布を返すのに対し、本エンドポイントは「1 日のうちどの時間帯にどれだけ
    観測が集まっているか」という周期パターンを 1 リクエストで返す。
    深夜バッチ / 業務時間帯 / 週末など、時間帯依存の稼働パターンを可視化する
    用途を想定する（`pulseboard-app/api-gateway/by_hour_of_day` および
    `trilingual-gateway/analytics-py/by_hour_of_day` と規約を揃える）。

    レスポンス:
        {
          "service": <service_name>,
          "total": <since/until 範囲内の全 observation 件数>,
          "distinct_hours": <populated な hour 数>,
          "by_hour_of_day": [
            {
              "hour": "00"〜"23",
              "count": N,
              "by_status": {healthy, unhealthy, degraded, unknown},
              "avg_response_ms": ..., "min_response_ms": ..., "max_response_ms": ...,
              "p50_response_ms": ..., "p95_response_ms": ..., "p99_response_ms": ...,
              "first_seen": <unix ts>, "last_seen": <unix ts>
            },
            ...
          ]
        }

    populated-only: 観測 0 件の時刻は配列に含めない（`timeseries` と同じスパース方針）。
    2 桁ゼロ詰め時刻 ("00"〜"23") は lex 順 = 時間順のため、返却順は自然にソート済み。
    対象サービスのレコードが範囲内に 1 件も無い場合は 404 を返す
    （`/metrics/services/{service_name}` 等と同じセマンティクス）。
    """
    if since is not None and until is not None and since > until:
        raise HTTPException(
            status_code=400,
